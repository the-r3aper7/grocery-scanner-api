...
class Instamart(ScraperBase):
    def __init__(
        self, lat: str, lon: str, scraper: CloudScraper = cloudscraper.create_scraper()
    ):
        self.base_url = "https://www.swiggy.com/api/instamart/search"
        self.scraper = scraper
        self.scraper.get("https://swiggy.com/")
    ...
    async def get_products(
        self, query: str, start: int = 0, size: int = 20
    ) -> List[Product]:
        params = {
            "pageNumber": "0",
                        ...
                        for variation in item.get("variations", []):
                            try:
                                ...
                                product = Product(
                                    ...
                                    max_in_cart=variation["cart_allowed_quantity"][
                                        "total"
                                    ],
                                ...
                                logging.log(
                                    logging.WARN,
                                    f"Error processing product variation: {e}",
                                )
                                continue
